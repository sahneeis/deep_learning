...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams,dropout_prob=0.1):
        super().__init__()
        ...
        self.input_size = hparams.get("input_size", 3 * 32 * 32)  # CIFAR-10 的默认输入大小
        self.num_classes = hparams.get("num_classes", 10)  # CIFAR-10 的默认类别数量
        self.n_hidden = hparams.get("n_hidden", 1000)  # 默认隐藏层大小   
        # # Model
        # self.linear_1 = nn.Linear(self.input_size, self.hparams["n_hidden"])
        # self.activation = nn.ReLU()
        # self.linear_2 = nn.Linear(self.hparams["n_hidden"], self.num_classes)
        ...
        self.model = nn.Sequential(
            ...
            nn.Linear(self.n_hidden, self.num_classes)
        )
    ...
    def forward(self, x):
        # # 确保 x 的形状是 [batch_size, 3, 32, 32]
        # assert len(x.shape) == 2 and x.shape[1] == 3072, f"Unexpected input shape: {x.shape}"
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.Adam(self.model.parameters(), lr=self.hparams['lr'],weight_decay=1e-4)
        pass
