...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding = Embedding(vocab_size=self.vocab_size, d_model=self.d_model, max_length=self.max_length)
        self.encoder = Encoder(d_model=self.d_model, d_k=self.d_k, d_v=self.d_v, n_heads=self.n_heads,
                       d_ff=self.d_ff, n=self.n, dropout=self.dropout)
        self.decoder = Decoder(d_model=self.d_model, d_k=self.d_k, d_v=self.d_v, n_heads=self.n_heads,
                       d_ff=self.d_ff, n=self.n, dropout=self.dropout)
        self.output_layer = nn.Linear(in_features=self.d_model, out_features=self.vocab_size, bias=False)
